...
class RTFM(nn.Module):
    def __init__(self, attention_type, n_features, batch_size, quantize_size, dropout):
        ...
        elif 'trans' in attention_type:
            self.attention = TransMIL(1, n_features)
        else:
            self.attention = None
    ...
    def get_mil_weights(self, features, batch_size, n_crops):
        ...
        # B x T x 1
        weights_scores = weights.view(batch_size, n_crops, -1, 1).mean(1)

        return weights_features, weights_scores

    def get_weighted_scores(self, scores, weights):
        # score 要在0-1之间
        weighted_scores = weights * scores
        weighted_scores = self.sigmoid(weighted_scores)
        # print(weighted_scores.shape)
        # pdb.set_trace()

        nor_score = weighted_scores[:self.batch_size]

        ab_score = weighted_scores[self.batch_size:]
        
        return nor_score, ab_score

    def get_weighted_features(self, features, weights, n_crops):
        # BN x C
        weighted_features = (features.permute(0, 2, 1) @ weights).squeeze(-1)

        nor_features = weighted_features[:self.batch_size*n_crops]

        ab_features = weighted_features[self.batch_size*n_crops:]

        return nor_features, ab_features
    ...
    def att_forward(self, inputs):
        ...
        weights_features, weights_scores = self.get_mil_weights(features, bs, ncrops)
        # BN x C
        weighted_features = (features.permute(0, 2, 1) @ weights_features).squeeze(-1)

        scores = self.relu(self.fc1(weighted_features))
        scores = self.drop_out(scores)
        scores = self.relu(self.fc2(scores))
        scores = self.drop_out(scores)
        scores = self.sigmoid(self.fc3(scores))
    ...
    def trans_forward(self, inputs):
        ...
        # B x T x 1
        scores_ins = results['scores_ins'].view(bs, ncrops, -1, 1).mean(1)

        features = results['features_ins']
        
        results = self.get_topk_features_scores(features, scores_ins, (bs, ncrops, t, features.shape[-1]))

        return dict(
        anomaly_score = scores_cls[self.batch_size:],
        regular_score = scores_cls[0:self.batch_size], 
        feature_select_anomaly = results['feature_select_anomaly'],
        feature_select_regular = results['feature_select_regular'],
        scores = scores_ins,
        feature_magnitudes = results['feature_magnitudes'])
    ...
    def forward(self, inputs):
        ...
        elif 'trans' in self.attention_type:
            result = self.trans_forward(inputs)
        else:
